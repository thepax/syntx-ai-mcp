...
__export(index_exports, {
  ...
  SyntxAPIError: () => SyntxAPIError,
  SyntxAbortError: () => SyntxAbortError,
  SyntxAuth: () => SyntxAuth,
  SyntxAuthError: () => SyntxAuthError,
  SyntxClient: () => SyntxClient,
  SyntxTimeoutError: () => SyntxTimeoutError,
  SyntxWebSocket: () => SyntxWebSocket,
  UserResource: () => UserResource,
  VideoResource: () => VideoResource,
  allTools: () => allTools,
  collectCompletedObjects: () => collectCompletedObjects,
  createMcpContext: () => createMcpContext,
...
var SyntxAuth = class {
  ...
  static generatePkcePair() {
    const codeVerifier = base64url((0, import_node_crypto.randomBytes)(32));
    const codeChallenge = base64url((0, import_node_crypto.createHash)("sha256").update(codeVerifier).digest());
...
function decodeBase64(input) {
  ...
  return import_node_buffer.Buffer.from(cleaned, "base64");
}
...
async function startHttp(opts) {
    ...
    async (req, res) => {
      ...
      if (req.method === "POST") {
        ...
        req.on("end", () => {
          ...
          let parsedBody;
          try {
            parsedBody = total > 0 ? JSON.parse(import_node_buffer2.Buffer.concat(chunks, total).toString("utf8")) : void 0;
          } catch {
            logSecurityEvent({
              kind: "transport.method.rejected",
              transport: "http",
  ...
  await new Promise((resolve2) => httpServer.listen(port, hostname, resolve2));
  return () => new Promise((resolve2) => {
...
function isAuthorized(headerValue, expected) {
  ...
  const provided = import_node_buffer2.Buffer.from(parts[1]);
  const expectedBuf = import_node_buffer2.Buffer.from(expected);
  if (provided.length !== expectedBuf.length) {
    (0, import_node_crypto2.timingSafeEqual)(provided, provided);
    return false;
  }
  return (0, import_node_crypto2.timingSafeEqual)(provided, expectedBuf);
}
